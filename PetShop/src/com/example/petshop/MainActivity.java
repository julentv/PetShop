...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		list.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			...
			@Override
			public boolean onItemLongClick(AdapterView<?> parent, View row,
	...
	public void createAnimalList() {
		arrAnimals = (new AnimalManager(getApplicationContext()))
				.loadAnimalsFromFile();
		if (arrAnimals == null) {
			arrAnimals = new ArrayList<Animal>();

		}
		adpAnimals = new ArrayAdapter<Animal>(this,
				android.R.layout.simple_list_item_activated_2,
				android.R.id.text1, arrAnimals);
	...
	public void onSearch(String text) {
		...
		for (int i = 0; i < arrAnimals.size(); i++) {
			if (arrAnimals.get(i).getName().toLowerCase()
		...
		adpAnimals = new ArrayAdapter<Animal>(this,
				android.R.layout.simple_list_item_activated_2,
				android.R.id.text1, newArrAnimals);
		list.setAdapter(adpAnimals);
		adpAnimals.notifyDataSetChanged();
	}
	...
	public void deleteAnimal() {
		...
		(new AnimalManager(getApplicationContext()))
				.deleteAnimal(animalPosition);
		this.createAnimalList();
	...
	private ActionMode.Callback mActionModeCallback = new ActionMode.Callback() {
		...
		public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
			// Disable the list to avoid selecting other elements while editing
			// one
		...
		public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
			...
			case R.id.mnu_animal_edit:
				mode.finish(); // Action picked, so close the CAB and execute
								// action
				// Edit event
				Intent intent = new Intent(MainActivity.this,
						NewAnimalActivity.class);
		...
		public void onDestroyActionMode(ActionMode mode) {
			...
			MainActivity.this.list.setItemChecked(MainActivity.this.position,
					false);
			mActionMode = null;
		}
	};
